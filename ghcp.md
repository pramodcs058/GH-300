# GitHub Copilot (GH-300)

A consolidated learning resource repository for **GitHub Copilot training participants**.  
This README contains official learning paths, hands-on labs, documentation, certification resources, and advanced references to help you master GitHub Copilot and the broader GitHub ecosystem.

---

# GitHub Copilot

GitHub Copilot is an AI-powered developer tool that helps accelerate software development through:

- AI-assisted code completion
- Code generation
- Intelligent chat-based assistance
- Refactoring recommendations
- Test generation
- Agent-based automation
- Context-aware development workflows

🔗 **Official GitHub Copilot**  
https://github.com/features/copilot

---

# GitHub Learning Path

Progress through the GitHub learning ecosystem in the recommended order:

## 1. GitHub Foundations (GH-900)
Build foundational GitHub knowledge.

https://learn.microsoft.com/en-us/training/courses/gh-900t00

---

## 2. GitHub Administration Basics (GH-100)
Learn repository governance, administration, and collaboration management.

https://learn.microsoft.com/en-us/training/courses/gh-100t00

---

## 3. Automate Your Workflow with GitHub Actions (GH-200)
Master CI/CD automation with GitHub Actions.

https://learn.microsoft.com/en-us/training/courses/gh-200t00

---

## 4. GitHub Copilot (GH-300)
Develop AI-assisted software engineering skills using GitHub Copilot.

https://learn.microsoft.com/en-us/training/courses/gh-300t00

---

## 5. GitHub Advanced Security (GH-500)
Learn code scanning, secret scanning, and supply chain security.

https://learn.microsoft.com/en-us/training/courses/gh-500t00

---

# GH-300 Core Resources

## Official Course
https://learn.microsoft.com/en-us/training/courses/gh-300t00

## Official Labs
https://microsoftlearning.github.io/mslearn-github/#gh-300

## GitHub Copilot Spaces
https://github.com/copilot/spaces

## GitHub Copilot Coding Agent
https://github.com/features/copilot/agents

---

# Hands-on Labs & Guided Exercises

## Build Auto Suggest Engine with Copilot
https://learn.microsoft.com/en-us/training/modules/build-auto-suggest-engine-copilot/

---

## Develop with AI-powered Code Suggestions using VS Code
https://github.com/skills/getting-started-with-github-copilot

---

## Democratize Tribal Knowledge using Copilot Spaces
https://github.com/skills/scale-institutional-knowledge-using-copilot-spaces

---

## Develop Unit Tests using GitHub Copilot
https://microsoftlearning.github.io/mslearn-github-copilot-dev/Instructions/Labs/LAB_AK_04_develop_unit_tests_xunit.html

---

## GitHub Skills Exercise
Build applications with Copilot Agent Mode

https://github.com/skills/build-applications-w-copilot-agent-mode

---

## Integrate MCP with GitHub Copilot
https://github.com/skills/integrate-mcp-with-copilot/

---

# GitHub Copilot Documentation

## Core Documentation

### GitHub Copilot Documentation
https://docs.github.com/en/copilot

### Best Practices
https://docs.github.com/en/copilot/get-started/best-practices

### AI Model Comparison
https://docs.github.com/en/copilot/reference/ai-models/model-comparison

### GitHub Copilot Plans
[https://docs.github.com/en/copilot/concepts/billing](https://github.com/features/copilot/plans)

### Billing
https://docs.github.com/en/copilot/concepts/billing

### Auto Model Selection
https://docs.github.com/en/copilot/concepts/auto-model-selection

---

# Copilot Chat Resources

## Copilot Chat Cookbook
https://docs.github.com/en/copilot/tutorials/copilot-chat-cookbook

## Copilot Chat Cheat Sheet
https://docs.github.com/en/copilot/reference/cheat-sheet

## Getting Started with Prompting
https://docs.github.com/en/copilot/how-tos/chat-with-copilot/get-started-with-chat

---

# Copilot Interaction Modes

## Ask Mode
https://docs.github.com/en/copilot/how-tos/chat-with-copilot/chat-in-ide#ask-mode

## Edit Mode
https://docs.github.com/en/copilot/how-tos/chat-with-copilot/chat-in-ide#edit-mode

## Agent Mode
https://docs.github.com/en/copilot/how-tos/chat-with-copilot/chat-in-ide#agent-mode

## Plan Mode
https://docs.github.com/en/copilot/how-tos/chat-with-copilot/chat-in-ide#plan-mode

---

# Advanced Copilot Capabilities

## Ask Questions in GitHub
https://docs.github.com/en/copilot/how-tos/chat-with-copilot/chat-in-github

## Customize Copilot Responses
https://docs.github.com/en/copilot/concepts/prompting/response-customization

## Copilot Code Review
https://docs.github.com/en/copilot/concepts/agents/code-review

## About Coding Agent
https://docs.github.com/en/copilot/concepts/agents/coding-agent/about-coding-agent

---

# MCP (Model Context Protocol)

## Extend Copilot Chat with MCP
https://docs.github.com/en/copilot/how-tos/provide-context/use-mcp/extend-copilot-chat-with-mcp

## MCP Servers Repository
https://github.com/modelcontextprotocol/servers/tree/main

## Enhance Agent Mode with MCP
https://docs.github.com/en/copilot/tutorials/enhance-agent-mode-with-mcp

## Workflow Transformation using MCP
https://github.blog/ai-and-ml/github-copilot/5-ways-to-transform-your-workflow-using-github-copilot-and-mcp/

---

# Copilot Spaces

## Collaborate using Copilot Spaces
https://docs.github.com/en/copilot/how-tos/provide-context/use-copilot-spaces/collaborate-with-others

---

# Customization & Productivity Tutorials

## Create README
https://docs.github.com/en/copilot/tutorials/customization-library/prompt-files/create-readme

## Creating Templates
https://docs.github.com/en/copilot/tutorials/copilot-chat-cookbook/communicate-effectively/creating-templates

## Document API
https://docs.github.com/en/copilot/tutorials/customization-library/prompt-files/document-api

## Generate Unit Tests
https://docs.github.com/en/copilot/tutorials/customization-library/prompt-files/generate-unit-tests

---

# Code Quality & Refactoring

## Create End-to-End Tests
https://docs.github.com/en/copilot/tutorials/copilot-chat-cookbook/testing-code/create-end-to-end-tests

## Decouple Business Logic from UI Components
https://docs.github.com/en/copilot/tutorials/copilot-chat-cookbook/refactor-code/decouple-business-logic

## Improve Code Readability
https://docs.github.com/en/copilot/tutorials/copilot-chat-cookbook/refactor-code/improve-code-readability

## Explore Feature Implementations
https://docs.github.com/en/copilot/tutorials/copilot-chat-cookbook/analyze-functionality/explore-implementations

---

# Security & Enterprise Practices

## Find Vulnerabilities
https://docs.github.com/en/copilot/tutorials/copilot-chat-cookbook/analyze-security/find-vulnerabilities

## Increase Test Coverage
https://docs.github.com/en/copilot/tutorials/roll-out-at-scale/drive-downstream-impact/increase-test-coverage

## Integrate Agentic AI into SDLC
https://docs.github.com/en/copilot/tutorials/roll-out-at-scale/enable-developers/integrate-ai-agents

## Manage Static Analysis Tools
https://docs.github.com/en/copilot/how-tos/use-copilot-agents/request-a-code-review/manage-tools

## Content Exclusion
https://docs.github.com/en/copilot/concepts/context/content-exclusion

---

# GH-300 Certification Resources

## Certification Overview
https://learn.microsoft.com/en-us/credentials/certifications/github-copilot/

---

## Study Guide
https://learn.microsoft.com/en-us/credentials/certifications/resources/study-guides/gh-300

---

## Practice Assessment
https://learn.microsoft.com/en-us/credentials/certifications/github-copilot/practice/assessment?assessment-type=practice&assessmentId=218035372&practice-assessment-type=certification

---

## Experience Demo
https://mscertdemo.starttest.com/

---

## Exam Duration & Exam Experience
https://learn.microsoft.com/en-us/credentials/support/exam-duration-exam-experience

Video Walkthrough:  
https://learn.microsoft.com/_themes/docs.theme/master/en-us/_themes/global/video-embed-one-stream.html?id=2c25fa81-b82a-45e5-a09d-d8a06e6c665a

---

## Exam Retake Policy
https://learn.microsoft.com/en-us/credentials/support/retake-policy

---

# Recommended Learning Sequence for Participants

Follow this sequence for best results:

### Phase 1 — Foundations
- GitHub Basics
- Repository Management
- Branching & Collaboration

### Phase 2 — Automation
- GitHub Actions
- CI/CD Workflows

### Phase 3 — Copilot Essentials
- Prompting
- Chat Modes
- Code Suggestions

### Phase 4 — Advanced Copilot
- Agent Mode
- Plan Mode
- MCP Integration
- Spaces

### Phase 5 — Certification Preparation
- Labs
- Practice Assessment
- Mock Tests

---

# Training Outcome

By completing these resources, participants will be able to:

- Use GitHub Copilot effectively in daily development
- Generate production-ready code faster
- Refactor and optimize code intelligently
- Automate development workflows
- Integrate AI into SDLC processes
- Prepare for and clear the **GH-300 GitHub Copilot Certification**

---

# Maintained For

**GitHub Copilot (GH-300) Training Participants**

Happy Learning and Happy Coding 🚀
